...
For breakdown or distribution questions, include percentage share of total when the user asks for percentage, share, or distribution.

For no-show analysis, include no-show rate percentage.

For Fathom coverage analysis, include Fathom coverage percentage.
...
SELECT
  ...
  ROUND(
    AVG(f.call_duration_seconds) FILTER (WHERE f.call_duration_seconds IS NOT NULL) / 60.0,
    2
