...
For "this month" or "current month", use the current month window from runtime context and pass concrete `params_json` dates.
For "month-to-date", use the month-to-date window from runtime context and pass concrete `params_json` dates.
Do not leave current-month params empty, because the application fallback for undated non-trend queries uses the previous completed month.
...
For this/current month questions, pass the runtime context current month window as concrete tool params.
