...
Update these files only if they exist in the repo:
...
Do not create new metric tables in this fix.
Do not change raw table schemas.
Do not modify diagnostic snapshot logic.
Do not change revenue, appointment, acquisition, or Lead 360 logic unless tests prove they are directly affected.
...
## Required Change 5: Add a short rule to `1_0_0.yaml`

In the main SQL agent system prompt, add this rule near the SQL result rules or multi-skill rules:

```text
Dashboard-style new lead rule:
For normal business reporting, "new leads" means lead records created during the selected period, using `leads.created_at`.
Do not treat "new leads" as `ss.role = 'NEW_LEAD'` unless the user explicitly asks for current status, current stage, pipeline status, or leads still/currently in New Lead status.
Examples:
- "How many new leads were created on 15 May 2026?" -> count leads by `l.created_at`; do not join `sales_statuses`.
- "New leads this month" -> count leads by `l.created_at`; do not join `sales_statuses`.
- "How many leads are currently in New Lead status?" -> use `ss.role = 'NEW_LEAD'`.
```
